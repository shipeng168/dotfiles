...
prompt_battery() {
  ...
  if [[ "$POWERLEVEL9K_BATTERY_VERBOSE" == true ]]; then
    message="$(($(cat $bat/batt_temp)/10))ºC  $remain$bat_percent%%"
  else
    message="$bat_percent%%"
  fi
  ...
  if [[ -n "$POWERLEVEL9K_BATTERY_LEVEL_BACKGROUND" ]] && [[ "${(t)POWERLEVEL9K_BATTERY_LEVEL_BACKGROUND}" =~ "array" ]]; then
    ...
    # Draw the prompt_segment
    "$1_prompt_segment" "$0_${current_state}" "$2" "${DEFAULT_COLOR}" "${battery_states[$current_state]}" "${message}"
  fi
}
...
prompt_dir() {
  ...
  "$1_prompt_segment" "$0_${current_state}" "$2" "blue" "$DEFAULT_COLOR" "${$(echo ${$(pwd)##*/}|sed -e "s,home,~,")}" "${dir_states[$current_state]}"
...
prompt_time() {
  ...
  "$1_prompt_segment" "$0" "$2" "black" "$DEFAULT_COLOR_INVERTED" "$time_format"
}
